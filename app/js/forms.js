$(function() {
  ...
  $('#modal1 #announcement-submit').click(function(event) {
    ...
    if (checkLength === true) {
      ...
      var path = ''
      var email
      var announcementContent = $('#announcementText')
        .val()
        .replace('&', 'and')
      ...
      for (i = 0; i < $formElements.length; i++) {
        ...
        serializedFormElements =
          serializedFormElements + $name + '=' + $val + '&'
      }

      $.each($uploads, function(key, value) {
        let fileName = value.name.split(' ').join('-')
      ...
      serializedData =
        ...
        email +
        '&uploads=' +
        fileNames //Concat all form input serialized data
      if (
        serializedData ===
        'announcementText=&email=jweigel@franciscan.edu&uploads='
      ) {
        return
      }
      ...
      request = $.ajax({
        url:
          'https://script.google.com/macros/s/AKfycbxZIsj-3jcq9KzJBM-q9eoyZcN5y1ITqXsa_H4tIaCyu26sGMA3/exec',
        type: 'post',
        data: serializedData,
      })
