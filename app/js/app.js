...
$(function () {
  ...
  if (globalToken || document.referrer.includes('https://accessfus.franciscan.edu/home')) {
    ...
    function renderCards(data, isotopeInit = true, isInfinite) {
      ...
      if (window.location.hash.toLowerCase() === "#all" || window.location.hash.indexOf('/') >= 0 || window.location.hash.split('#')[1] === "" || window.location.hash === "" || path === 'time-sensitive') {
       ...
       `;
              ...
              if (post.acf.atachments !== 'false' && post.acf.attachments !== false) {
                if (post.acf.attachments !== undefined && post.acf.atachments !== "") {
                  let singleAttachmentTemp = '';
                  let attachmentArr = post.acf.attachments;
                  for (let attachment of attachmentArr) {
                    singleAttachmentTemp = `
            ${singleAttachmentTemp}
               <a href="${attachment.attachment.url}"><i class="material-icons">attachment</i> ${attachment.attachment.title}</a>
           `;
                  }
       ...
       <div class="card isotope-item ${tagIds}">
          ...
          ${attachmentTemp}
            ...
            if (i === data.length - 1) {
              ...
              // Append help link
              if ($('#help').length === 0) {
                $('.side-nav').append(`<li><a href="https://franciscan.university/bulletin/help" title="help" id="help" target="_blank" >Help</a></li>`);
              }
